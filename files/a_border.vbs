...
'-----------------------------------------------------------------------
Private Sub usage()
	Wscript.Echo "Active注残データ"
	Wscript.Echo "a_border.vbs [option]"
	...
	Wscript.Echo " /debug"
	Wscript.Echo "Ex."
	Wscript.Echo "a_border.vbs /db:newsdc-4 /load "
	Wscript.Echo "----"
	Wscript.Echo "CDate(41445)=" & CDate(41445)
End Sub
'-----------------------------------------------------------------------
'メイン
'-----------------------------------------------------------------------
...
Private Function Load(byval strFilename)
	...
	dim	objRs
	set objRs = OpenRs(objDb,"a_border")
	Call ExecuteAdodb(objDb,"delete from a_border")
	dim	objFSO
	...
	do while ( objFile.AtEndOfStream = False )
		...
		if cnt > 1 then
			...
			for each c in (aryBuff)
				if i > 0 then
					Call Debug(i & ":" & objRs.Fields(i-1).Name & ":" & c)
					select case i
					case 2,6,17,28,29,30,33,34,35,36,45,53,54,55,56,57,58
						c = RTrim(Left(c,1))
					case 18
						c = RTrim(Left(c,2))
					case 9
						c = RTrim(Left(c,20))
					case 19,20,21,22,62,65
						if c <> "" then
							c = Replace(CDate(c),"/","")
						end if
					end select
