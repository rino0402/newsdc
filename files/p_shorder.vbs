...
sqlStr = "select *"
sqlStr = sqlStr & " from P_SHORDER"
sqlStr = sqlStr & " where G_SYUSHI in ('710','720','730','740','750')"
...
Do While Not rsSrcPSHOrder.EOF
	...
	strHinGai = rtrim(rsSrcPSHOrder.Fields("HIN_GAI"))

	strBuff = "SRC"
	strBuff = strBuff & " " & rsSrcPSHOrder.Fields("ORDER_NO")
	strBuff = strBuff & " " & rsSrcPSHOrder.Fields("ORDER_DT")
	strBuff = strBuff & " " & strHinGai
	strBuff = strBuff & " " & rsSrcPSHOrder.Fields("ORDER_QTY")
	strBuff = strBuff & " " & rsSrcPSHOrder.Fields("G_SYUSHI")
	...
	for each f in rsSrcPSHOrder.Fields
		...
		case "G_SYUSHI"
			if strJgyobu = "S" then
				select case f
				case "710"
					rsDstPSHOrder.Fields(f.Name) = "130"
				case "720"
					rsDstPSHOrder.Fields(f.Name) = "140"
				case "730"
					rsDstPSHOrder.Fields(f.Name) = "202"
				case "740"
					rsDstPSHOrder.Fields(f.Name) = "220"
				case "750"
					rsDstPSHOrder.Fields(f.Name) = "240"
				case else
					rsDstPSHOrder.Fields(f.Name) = "999"
				end select
			end if
		case "ORDER_NO"
		...
		case "HIN_GAI"
			rsDstPSHOrder.Fields(f.Name) = strHinGai
		case else
