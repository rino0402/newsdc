...
def main(r):
    print("main({})".format(r))
    log = os.path.dirname(os.path.abspath(__file__)) + '\\AcShort.log'
    try:
...
#    print(df)
    ...
    print(df)
    print("delete from AcShort", end=".")
    conn.execute("delete from AcShort")
...
#        print(i,row)
        data = {}
        data["Id_No"] = str(row["ID-NO"])
        data["Pn"] = str(row["品目番号"])
        data["Toku"] = str(row["得意先コード"])
        data["Choku"] = str(row["直送相手先コード"])
        data["Qty"] = int(row["出荷予定数"])
...
def insert(conn, data):
    sql = "insert into AcShort (" + ",".join(map(str, data)) + ") values ("
    for d in data:
        if type(data[d]) == str:
