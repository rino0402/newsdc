...
def main(r):
    print("main({})".format(r))
    log = os.path.dirname(os.path.abspath(__file__)) + '\\AcZan.log'
    try:
...
#    print(df)
    ...
    conn.execute("delete from AcZan")
...
#        print(i,row)
        data = {}
        data["ID"] = str(row["発注納入管理番号"])
        data["Pn"] = row["品目番号"]
        data["Qty"] = row["入出庫予定数"]
        data["SQty"] = row["正味入出庫予定数"]
...
def insert(conn, data):
    sql = "insert into AcZan (" + ",".join(map(str, data)) + ") values ("
    for d in data:
        if type(data[d]) == str:
